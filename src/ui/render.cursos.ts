...
type SortKey = keyof Curso;
type SortDir = "asc" | "desc";

let sortKey: SortKey = "id";
let sortDir: SortDir = "asc";
let currentQuery = "";
let currentEstado: "todos" | "disponible" | "cerrado" = "todos";
let editingId: number | null = null;

export function initCursosUI(svcC: CursoService, svcI: InscripcionService): void {
  ...
  document.getElementById("modal-curso")?.addEventListener("click", (e) => {
    if ((e.target as HTMLElement).id === "modal-curso") closeModal("modal-curso");
  });
...
function handleSubmitCurso(svcC: CursoService): void {
  const nombre  = getVal("cur-nombre");
  const sigla   = getVal("cur-sigla");
  const docente = getVal("cur-docente");
  const cupoStr = getVal("cur-cupo");
  const estado  = getVal("cur-estado") as "disponible" | "cerrado";
  const cupo    = parseInt(cupoStr, 10);

  const v = runValidations(
    notEmpty(nombre,  "Nombre"),
    notEmpty(sigla,   "Sigla"),
    notEmpty(docente, "Docente"),
    notEmpty(cupoStr, "Cupo máximo"),
    validCupo(isNaN(cupo) ? -1 : cupo)
  );
...
export function renderTablaCursos(svcC: CursoService, svcI: InscripcionService): void {
  ...
  tbody.querySelectorAll("[data-action]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (action === "edit-cur")   handleEditCurso(id, svcC);
      if (action === "toggle-cur") handleToggleCurso(id, svcC);
      if (action === "del-cur")    handleDeleteCurso(id, svcC, svcI);
    });
  });
}

function handleEditCurso(id: number, svcC: CursoService): void {
...
function handleToggleCurso(id: number, svcC: CursoService): void {
  try {
...
function handleDeleteCurso(id: number, svcC: CursoService, svcI: InscripcionService): void {
  const c = svcC.getById(id);
  if (!c) return;
  showConfirm(
    `¿Eliminar el curso "${c.nombre}"?`,
    "Se eliminarán también todas sus inscripciones. Esta acción no se puede deshacer.",
    () => {
      try {
