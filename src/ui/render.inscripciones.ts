...
export function initInscripcionesUI(
  ...
  document.getElementById("btn-nueva-inscripcion")?.addEventListener("click", () => {
    ...
    (document.getElementById("form-inscripcion") as HTMLFormElement)?.reset();
  ...
  document.getElementById("modal-inscripcion")?.addEventListener("click", (e) => {
    if ((e.target as HTMLElement).id === "modal-inscripcion")
      closeModal("modal-inscripcion");
  });
...
function populateSelects(svcE: EstudianteService, svcC: CursoService): void {
  const selEst = document.getElementById("ins-estudiante") as HTMLSelectElement;
  const selCur = document.getElementById("ins-curso") as HTMLSelectElement;
  if (!selEst || !selCur) return;
...
function handleSubmitInscripcion(svcI: InscripcionService): void {
  const estId = Number((document.getElementById("ins-estudiante") as HTMLSelectElement).value);
  const curId = Number((document.getElementById("ins-curso") as HTMLSelectElement).value);
...
export function renderTablaInscripciones(
  svcE: EstudianteService,
  svcC: CursoService,
  svcI: InscripcionService
): void {
  ...
  tbody.querySelectorAll("[data-action]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (el.dataset["action"] === "cancel-ins") handleCancelInscripcion(id, svcI);
      if (el.dataset["action"] === "del-ins")    handleDeleteInscripcion(id, svcI);
    });
  });
}

function handleCancelInscripcion(id: number, svcI: InscripcionService): void {
  showConfirm(
    "¿Cancelar esta inscripción?",
    "La inscripción pasará a estado 'cancelada' pero permanecerá en el historial.",
    () => {
      try {
        svcI.cancelar(id);
...
function handleDeleteInscripcion(id: number, svcI: InscripcionService): void {
  showConfirm(
...
export function refreshRelacionesSelects(
  svcE: EstudianteService,
  svcC: CursoService
): void {
  const selEst = document.getElementById("rel-select-estudiante") as HTMLSelectElement;
  const selCur = document.getElementById("rel-select-curso") as HTMLSelectElement;
...
function renderCursosDeEstudiante(
  estudianteId: number,
  svcC: CursoService,
  svcI: InscripcionService
): void {
  ...
  if (!estudianteId) { list.innerHTML = ""; return; }
...
function renderEstudiantesEnCurso(
  cursoId: number,
  svcE: EstudianteService,
  svcI: InscripcionService
): void {
  ...
  if (!cursoId) { list.innerHTML = ""; return; }
