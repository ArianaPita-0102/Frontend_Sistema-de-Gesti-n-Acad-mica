...
export function renderDashboard(
  ...
  if (top && banner) {
    ...
    if (curso) {
      ...
      if (nameEl)  nameEl.textContent  = `${curso.sigla} — ${curso.nombre}`;
      if (countEl) countEl.textContent = `${top.count} estudiante${top.count !== 1 ? "s" : ""} inscritos`;
      if (topEl)   topEl.textContent   = `${curso.sigla}`;
    }
  } else {
...
function renderOcupacionCursos(svcC: CursoService, svcI: InscripcionService): void {
...
function setText(id: string, value: number | string): void {
