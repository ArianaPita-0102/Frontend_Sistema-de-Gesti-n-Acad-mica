...
import { initInscripcionesUI, renderTablaInscripciones, refreshRelacionesSelects, } from "./ui/render.inscripciones.js";
import { renderDashboard } from "./ui/render.dashboard.js";
import { showSection, toggleDarkMode, loadDarkMode, showToast, } from "./ui/ui.helpers.js";
...
async function loadInitialData() {
    ...
    if (localStorage.getItem("sga_estudiantes") ||
    ...
    try {
        ...
        if (json.estudiantes)
            svcE.importData(json.estudiantes);
        if (json.cursos)
            svcC.importData(json.cursos);
        if (json.inscripciones)
            svcI.importData(json.inscripciones);
...
function initIO() {
    ...
    document.getElementById("input-import")?.addEventListener("change", async (e) => {
        ...
        try {
            ...
            if (raw.estudiantes)
                svcE.importData(raw.estudiantes);
            if (raw.cursos)
                svcC.importData(raw.cursos);
            if (raw.inscripciones)
                svcI.importData(raw.inscripciones);
